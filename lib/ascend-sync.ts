...
function serializeState(userId: string, state: PersistedAscendState): UserStateRow {
  return {
    user_id: userId,
...
function deserializeState(row: UserStateRow | null): PersistedAscendState {
  if (!row) {
...
export async function loadRemoteAscendState(session: Session | null) {
  ...
  const { data, error } = await supabase
    .from("user_rpg_state")
    .select(
      "user_id, selected_path_id, active_mission_id, selected_activity_type, xp_by_node, session_feed"
    )
  ...
  return deserializeState(data ?? null);
}
...
export async function saveRemoteAscendState(
  ...
  const payload = serializeState(session.user.id, state);
